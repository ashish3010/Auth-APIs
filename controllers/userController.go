...
func VerifyPassword(userPassword string, providedPassword string) (bool, string) {
	...
	if err != nil {
		msg = fmt.Sprintf("email or password is invalid")
		check = false
	}
	return check, msg
}
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := c.BindJSON(&user); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Error occured on Bind" + err.Error()})
			return
		}
		validationErr := validate.Struct(user)
		if validationErr != nil {
			log.Panic("Error occured while validation ", validationErr)
